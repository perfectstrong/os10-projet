...
\begin{algorithm}
  ...
  \ForEach{$\alpha_i \in A$}{
    ...
    \While{$\sigma \neq \emptyset$}{
      ...
      Chercher $o_{Max}$ l'opération qui augmente le plus $C_max$ (de $s_{i}$); et $o_{min}$ l'opération qui augmente le moins $C_max$ en tenant compte les maintenances dont les temps de complétion doivent se situer dans les intervalles donnés $\left[t_{jl}^E, t_{jl}^L\right]$ \;
